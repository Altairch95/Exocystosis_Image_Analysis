...
__credits__ = ["Oriol Gallego", "Radovan Dojcilovic"]
__version__ = "1.0"
...
if __name__ == "__main__":
    data_path = "../2022-MAR/1103/trackpy/"
    data_dict = dict()
    for file_path in glob.glob(data_path + "detected*"):
        if file_path.endswith("csv"):
            data = Biodata(file_path)
            info = data.data
            info2 = data.get_data(columns=[0, 1], names={'y': 'x', 'x': 'y'})
            data_dict.setdefault(data.name, data.data)
    x = 0
