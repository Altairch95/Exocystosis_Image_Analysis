...
Warnings:
    - Trackpy sometimes gives an initial error when opening the images.
    Modify tiff_file.py with this, if not an error came out:
            return datetime(year=int(dt_str[0:4]), month=int(dt_str[4:6]),
                        day=int(dt_str[6:8]), hour=int(dt_str[9:11]),
                        minute=int(dt_str[12:14]), second=int(dt_str[15:17]))
