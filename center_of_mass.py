...
from ..masks import (binary_mask, r_squared_mask, r_squared_mask_custom,  # r_squared_mask_custom is a custom function
...
def refine_com(raw_image, image, radius, coords, max_iterations=10,
    ...
    # custom DataFrame([x, y, mass, size, ecc, signal, raw_mass, m0, m1, m2, m3, m4, m5])
        where mX are the different moments of intensities of the refined blob calculated from
        the neighborhood mask of the blob.
    """
    ...
    if characterize:
        ...
        columns += default_size_columns(image.ndim, isotropic) + \
            ['ecc', 'signal', 'raw_mass', "m0", "m2",
             "mu11", "mu20", "mu02"] # custom: add colum m0 - m5 for
        # caculating the moments of intensity and mu11, mu20, mu02
...
def refine_com_arr(raw_image, image, radius, coords, max_iterations=10,
    ...
    if engine == 'python':
        ...
        # custom: if 'python' this function ends here --> go to 'return results'
    elif engine == 'numba':
...
def _refine(raw_image, image, radius, coords, max_iterations,
    ...
    if characterize:
        if isotropic:
            Rg = np.empty(N, dtype=np.float64)  # custom: RG is size --> moment of intensity m2
            # custom: Defining moments of intensities
            m0 = np.empty(N, dtype=np.float64)  # custom: to calculate momentum of intensity m0
            m2 = np.empty(N, dtype=np.float64)  # a copy of RG, which is not modified
            mu11 = np.empty(N, dtype=np.float64)
            mu20 = np.empty(N, dtype=np.float64)
            mu02 = np.empty(N, dtype=np.float64)
        else:
    ...
    for feat, coord in enumerate(coords):
        ...
        # custom: normalized nerighborhood to calculate m0

        # We first calculate the number neighbourhood normalized, which is already applied with binary mask:
        imax = np.amax(image)
        imax_2 = image.max()
        # neighborhood_norm = np.divide(neighborhood, imax)
        neighborhood_norm = neighborhood
        mass[feat] = neighborhood_norm.sum()

        # We now calculate the moments of intensity:
        moments = cv.moments(neighborhood_norm)
        # m0[feat] = neighborhood_norm.sum()
        result = np.where(image == np.amax(image))

        # Defining each intensity moment
        m0[feat] = moments["m00"]  # the same as mass
        m2[feat] = (moments["mu20"] + moments["mu02"])
        mu11[feat] = moments["mu11"] / moments["m00"]
        mu20[feat] = moments["mu20"] / moments["m00"]
        mu02[feat] = moments["mu02"] / moments["m00"]

        # custom ecc
        tetha = np.sqrt((mu20[feat] - mu02[feat])**2 + (4 * (mu11[feat])**2))
        # ecc_pt[feat] = (mu02[feat] + mu20[feat] + tetha) / (mu02[feat] + mu20[feat] - tetha)

        # custom orientation
        # ori[feat] = 2 * mu11[feat] / (mu20[feat] - mu02[feat])

        if not characterize:
            continue  # short-circuit loop
        if isotropic:
            # Rg[feat] = np.sqrt(np.sum(r_squared_mask(radius, ndim) * neighborhood_norm) / mass[feat])
            Rg[feat] = np.sum(r_squared_mask(radius, ndim) * neighborhood_norm) / mass[feat]

        else:
            Rg[feat] = np.sqrt(ndim * np.sum(x_squared_masks(radius, ndim) * neighborhood,
                                             axis=tuple(range(1, ndim + 1))) / mass[feat])

        # I only know how to measure eccentricity in 2D.
        if ndim == 2:
            ecc[feat] = np.sqrt(np.sum(neighborhood_norm*cosmask(radius))**2 +
                                np.sum(neighborhood_norm*sinmask(radius))**2)
            ecc[feat] /= (mass[feat] - neighborhood_norm[radius] + 1e-6)
            ...
            ecc[feat] = np.nan
        signal[feat] = neighborhood_norm.max()  # custom: using norm neighbourhood
    ...
    if not characterize:
        ...
        # custom: return a column stack with all the moments of intensity
        return np.column_stack([final_coords, mass, Rg, ecc, signal, raw_mass, m0, m2, mu11, mu20, mu02]) # custom
        # return np.column_stack([final_coords, mass, Rg, ecc, signal, raw_mass])  # original trackpy
