...
def outlier_rejection(results_dir, figures_dir, test=False, mu_ini=None, sigma_ini=None):
    """
    ...
    as described in Eq. 4 (Churchman, 2005).
    ...
    if not os.path.exists(results_dir + 'kde/') or len(os.listdir(results_dir + 'kde/')) == 0:
        sys.stderr.write('\nPICT-MODELLER-ERROR: Hey! Good morning! Are you trying to build the house from the roof?\n'
                         'Go and climb a rock!! Just kidding, of course :)\n'
