...
def convert_to_image(rlefile, outputdirectory, preprocessed_image_list,
    ...
    for f in files:
        ...
        # Custom: retrieve contour
        image_c = image.astype(np.uint8)
        contour = find_boundaries(image_c, mode='outer').astype(np.uint8)  # find contour as 1 in the surface
        contour_2 = expand_labels(image_c, 1)
        contour_3 = mark_boundaries(image, image_c, mode="outer")
        # coords = (np.where(contour > 0))
        # contour_coords = np.array(list(zip(coords[0], coords[1])))
        imsave(outputdirectory + "contour_image_01.tif", contour, plugin="tifffile", check_contrast=False)
        # imsave(outputdirectory + "contour_image_01_2.tif", contour_2, plugin="tifffile", check_contrast=False)
